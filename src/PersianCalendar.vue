  ...
  <div id="persian-calendar" dir="rtl">
    ...
    <div id="vpc_header" slot="header">
      <div id="vpc_date-control">
        ...
        <div v-if="!disableToday" class="vpc_today-btn" @click="goToday" :disabled="todayBtnDisable">
          امروز
        </div>
      </div>
    ...
    <!--CALENDAR GRID-->
    <transition :name="transitionAction" @after-leave="afterLeave">
      <div v-if="currentDateChange">
        <div id="vpc_calendar">
          <!--DAYS HEADER-->
          <div id="vpc_days-header">
            <div>
              {{ $moment().isoWeekday(0).format('dddd') }}
            </div>
            <div>
              {{ $moment().isoWeekday(1).format('dddd') }}
            </div>
            <div>
              {{ $moment().isoWeekday(2).format('dddd') }}
            </div>
            <div>
              {{ $moment().isoWeekday(3).format('dddd') }}
            </div>
            <div>
              {{ $moment().isoWeekday(4).format('dddd') }}
            </div>
            <div>
              {{ $moment().isoWeekday(5).format('dddd') }}
            </div>
            <div>
              {{ $moment().isoWeekday(6).format('dddd') }}
            </div>
          </div>
          <!--WEEKS ROW-->
          <div v-for="week in periodWeeks" :key="week.uid" :class="weekClassObject">
              ...
              >
                ...
                >
                  ...
                  <span class="vpc_event-start-time" v-if="!hideEventTimes"
                    ...
                    }}<span> - {{ i.endDateTime.format('HH:mm').toPersianDigits() }} </span>
                  </span>
                  <span class="vpc_event-start-date" v-if="!hideEventTimes">{{
                    i.startDateTime.format('MM/DD').toPersianDigits()
                  }}</span>
                  ...
                  <span class="vpc_event-end-date" v-if="!hideEventTimes">{{
                    i.endDateTime.format('MM/DD').toPersianDigits()
                  }}</span>
                </div>
              </slot>
            </template>
...
export default {
  ...
  computed: {
    ...
    daysInWeek() {
      const days = [],
        day = this.currentDate.clone().startOf('Week')
      do {
        days.push(day.clone())
        day.add(1, 'days')
      } while (!day.isSame(day.clone().startOf('week')))
      ...
      if (this.min) {
        if (this.max)
          return !this.$moment().isBetween(
            this.$moment(this.min).startOf('day'),
            this.$moment(this.max).endOf('day')
          )
  ...
  methods: {
    ...
    goToday() {
      ...
      if (
        this.currentDate.isBefore(this.$moment(), 'month') ||
        (this.isWeekPeriod && this.currentDate.isBefore(this.$moment().startOf('Week')))
      ) {
        this.transitionAction = 'slide-left'
        this.currentDate = this.$moment()
      } else if (
        this.currentDate.isAfter(this.$moment(), 'month') ||
        (this.isWeekPeriod && this.currentDate.isAfter(this.$moment(), 'week'))
      ) {
        this.transitionAction = 'slide-right'
        this.currentDate = this.$moment()
      }
    ...
    dayClassObject(day) {
      ...
      return {
        ...
        'vpc_not-current-month':
          !day.isSame(this.currentDate, 'month') && !this.isWeekPeriod && !this.hideMonthShadow,
    ...
    findAndSortEventsInWeek(weekStart) {
      ...
      return this.findAndSortEventsInDateRange(
        weekStart,
        this.$moment(weekStart).hours(23).minutes(59).seconds(59).add(6, 'days')
      )
    },
    findAndSortEventsInDateRange(startDateTime, endDateTime) {
      ...
      return this.events
        .filter(
          ...
          (event) =>
            event.endDateTime.isSameOrAfter(startDateTime) && event.startDateTime.isSameOrBefore(endDateTime),
          this
        )
    ...
    dayEvents(day) {
      return this.events
        .filter((d) => d.endDateTime.isSameOrAfter(day) && d.startDateTime.isSameOrBefore(day))
        .sort(this.eventComparer)
    },
    dayIsSelected(day) {
    ...
    getWeekEvents(weekStart) {
      ...
      for (let i = 0; i < events.length; i++) {
        ...
        const spanContinued = continued
          ? this.diff(this.$moment(ep.endDateTime), weekStart) + 1
          : this.diff(this.$moment(ep.endDateTime), ep.startDateTime) + 1
        ...
        if (
          ep.endDateTime.isBetween(
            this.$moment(ep.startDateTime),
            this.$moment(ep.startDateTime).endOf('day'),
            undefined,
            '[]'
          )
        )
